...
addpath('C:\Users\User\Desktop\FPM_INR-main\FPM_INR-main\')
%% Operator
...
if if_3D
    sample_name = 'sheepblood'; % 'sheepblood', 'WU1005', 'Siemens'
...
for dz = 1:length(dz_set)
    ...
    kxx = kxx - mean(mean(kxx));
    kyy = kyy - mean(mean(kyy));
    ...
    for iter=1:Niter
        ...
        for led_num=1:ID_len
            ...
            if ((mean(mean(oI_cap)) > 0.1 && mean(mean(oI_bef)) > 0.1) || (mean(mean(oI_cap)) < 0.1 && mean(mean(oI_bef)) < 0.1))
    ...
    %%
    figure(1000);
    subplot(1,2,1), imagesc(oI/max(oI(:))), axis image, colormap gray; axis on; title('Amplitude');colorbar;
    subplot(1,2,2), imagesc(oP), axis image, colormap gray; axis on; caxis([-pi,pi]);title('Phase');colorbar;
